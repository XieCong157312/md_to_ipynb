...
class NBStructure(dict):
    ...
    def __init__(self, name):
        ...
        self.update(
            (("metadata", {}), ("nbformat", 4), ("nbformat_minor", 2), ("cells", []))
        )
        self["metadata"].update(
            {"language_info": {"name": "python"}, "orig_nbformat": 4}
        )
...
class NBMarkdownCell(dict):
    ...
    def __init__(self):
        ...
        self.update((("cell_type", "markdown"), ("metadata", {}), ("source", [])))
...
class IPyNB(object):
    ...
    def load(self, string):
        ...
        segments = re.split(
            "(```.*?```)",
            string.replace("```python", "```").replace("```py", "```"),
            flags=re.DOTALL,
        )
